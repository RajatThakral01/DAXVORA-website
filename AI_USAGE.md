...
- **Manual verification — human-performed (not agent-simulated):** to be performed before Session 18 is considered verified — keyboard Tab through all controls (scenario buttons, Confirm governance, Decide action, Simulate source failure, Restart) with visible focus; screen-reader announcement of the `aria-live` region on status/decision/failure transitions; and `source_failed` styling reading as distinct from a normal terminal state without obscuring `role="alert"` — same checklist as Session 16; results to be logged here once human completes the pass.
- **Generated vs. human-authored:** all code agent-generated; visual treatment choices are agent-implemented within DESIGN v3 §6/§7 bounds; manual verification results will be human-performed and logged verbatim once provided, not agent-simulated.
